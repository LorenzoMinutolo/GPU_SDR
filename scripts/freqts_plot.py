...
import argparse
def run(backend, files, decimation, low_pass, channel_list, displayed_samples, start_time, end_time):
    u.plot_frequency_timestreams(files, decimation=decimation, low_pass=20, backend=backend, output_filename=None,
              channel_list=ch_list, auto_open=True, displayed_samples = displayed_samples, start_time=start_time, end_time=end_time)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start', '-s', help='Start time for plotting', type=int, default = None)
    parser.add_argument('--end', '-e', help='End time for plotting', type=int, default = None)
    ...
    run(backend = args.backend, files = files, decimation = args.decimation, displayed_samples = args.displayed_samples, channel_list = ch_list, low_pass = args.lowpass, start_time = args.start, end_time = args.end)
